...
def main():
    ...
    action.add_argument("-m", "--metadata", action="store_true", help="Dump book metadata from of a .kfx file as .json")
...
def get_kindle_kfx_metadata(filepath):
    ...
    return extract_metadata(KFXContainer(packed_data).decode())
...
class KFXContainer(PackedBlock):
    ...
    def decode(self):
        ...
        return [entity.decode(symtab) for entity in self.entities]
